...
# Lazy load YOLO model (don't load at startup to avoid Cloud Run timeout)
YOLO_MODEL = None
...
def get_yolo_model():
    ...
    if YOLO_MODEL is None:
        ...
        if not torch.cuda.is_available():
            YOLO_MODEL.model.cpu()
        print("YOLO model loaded successfully")
    return YOLO_MODEL

url: str = os.getenv("SUPABASE_URL")
key: str = os.getenv("SUPABASE_KEY")
depth_url: str = os.getenv("DEPTH_URL")
...
class FootPath(Resource):
    def post(self):
        try:
            ...
            # YOLO segmentation prediction for footpath detection
            ...
            if result.masks is not None:
                ...
                # Store bounding boxes for detections (used in walkability calculation)
                if len(boxes_data) > 0:
                    # Create a simple detections object with xyxy attribute and __len__ method
                    class Detections:
                        def __init__(self, xyxy):
                            self.xyxy = xyxy
                        def __len__(self):
                            return len(self.xyxy)
                    detections = Detections(boxes_data)
                
                # Process each mask
                for mask_data in masks_data:
                    # Resize mask to original image size
                    mask_resized = cv2.resize(mask_data, (image_cv.shape[1], image_cv.shape[0]), interpolation=cv2.INTER_LINEAR)
            ...
            def make_request():
                try:
                    with open(image_path, 'rb') as img_file:
                        ...
                        # Make request to auth-tagging endpoint on same service
                        # Use localhost on the same port (8080) since it's the same service
                        service_url = os.getenv('AUTH_TAGGING_URL', 'http://localhost:8080/auth-tagging')
                        response = requests.post(
                            service_url,
                            files=files, 
                            data=data,
...
def health_check():
    ...
    return jsonify({'status': 'healthy', 'service': 'pathfinders-backend'}), 200
